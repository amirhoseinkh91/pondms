...
public class JalaliCalendar extends Calendar {
    ...
    private CalendarType ct;
    public JalaliCalendar(CalendarType ct)
    {
    	this.ct = ct;
    	this.monthNumberMap = getMonthNumberMap(ct);
    }
    public JalaliCalendar(CalendarType ct, Map<String,Integer> monthNumberMap)
    {
    	this.ct = ct;
    	this.monthNumberMap = monthNumberMap;
    }
    public Date getDate(String year,String month)
    {
    	if(ct.equals(CalendarType.SHAMSI))
    		return getGregorianDate(String.format("%s/%s/1",year,monthNumberMap.get(month))); 
    	else
    	{
    		Calendar c = Calendar.getInstance();
    		int m = monthNumberMap.get(month.replaceAll("//s+",""));
    		c.set(Integer.valueOf(year.replaceAll("//s+","")),m-1,1);
    		return c.getTime();
    	}
    }
    ...
    {
    	if(ct.equals(CalendarType.SHAMSI))
    		return getGregorianDate(String.format("%s/1/1",year)); 
    	else
    	{
    		Calendar c = Calendar.getInstance();
    		c.set(Integer.valueOf(year.replaceAll("//s+","")),0,1);
    		return c.getTime();
    	}
    }
    private Map<String,Integer> monthNumberMap;
    private Map<String,Integer> getMonthNumberMap(CalendarType ct)
    {  	
    	String[] shamsiMonths = {"far","ord","khr","tir","mor","shr","mhr","abn","azr","dey","bah","esf"};
    	String[] miladiMonths = {"JANUARY","FEBRUARY","MARCH","APRIL","MAY","JUNE","JULY","AUGUST"
    							,"SEPTEMBER","OCTOBER","NOVEMBER","DECEMBER"};
    	
    	monthNumberMap = new HashMap<>();
    	
    	if(ct.equals(CalendarType.SHAMSI))
    		for(int i=0;i<12;i++)
    			monthNumberMap.put(shamsiMonths[i], i + 1);		
    	else
    		for(int i=0;i<12;i++)
    			monthNumberMap.put(miladiMonths[i], i + 1);
	
    	return monthNumberMap;
    }
}
