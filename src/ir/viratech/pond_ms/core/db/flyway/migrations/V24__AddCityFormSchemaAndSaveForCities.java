...
public class V24__AddCityFormSchemaAndSaveForCities extends BaseJdbcMigration {
	...
	private void addCityFormsToCities() throws ValidationException, EntityTypeNotFoundException {
		...
		for (ParentLayer layer : layers) {
			if (layer.getType().equals(Layer.TYPE_PARENT) && layer.getName().startsWith("شهر")) {
				...
				pointCoordinateArray.add(p.getPoint().getX());
				pointCoordinateArray.add(p.getPoint().getY());
				point.put("coordinates", pointCoordinateArray);
