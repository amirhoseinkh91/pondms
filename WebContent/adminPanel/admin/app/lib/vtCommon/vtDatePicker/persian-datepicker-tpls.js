...
.directive('persianMonthpicker', [ 'dateFilter', 'PersianDateService', 'persianDateFilter', function(dateFilter, PersianDateService, persianDateFilter) {
	return {
		...
		link : function(scope, element, attrs, ctrl) {
			...
			ctrl.compare = function(date1, date2) {
				var shamsiYearStr1 = moment(date1).format('jYYYY/jM/jD').split("/")[0];
				var shamsiYearStr2 = moment(date2).format('jYYYY/jM/jD').split("/")[0];
				var shamsiMonthStr1 = moment(date1).format('jYYYY/jM/jD').split("/")[1];
				var shamsiMonthStr2 = moment(date2).format('jYYYY/jM/jD').split("/")[1];
				
				if((shamsiYearStr1 == shamsiYearStr2) && (shamsiMonthStr1==shamsiMonthStr2)){
					return 0;
				}else{
					if((parseInt(shamsiYearStr1) - parseInt(shamsiYearStr2))==0){
						return (parseInt(shamsiMonthStr1) - parseInt(shamsiMonthStr2))
					}else{
						return (parseInt(shamsiYearStr1) - parseInt(shamsiYearStr2))
					}
				}
	...
	return {
		...
		link : function(scope, element, attrs, ctrl) {
			...
			ctrl.compare = function(date1, date2) {
				...
				var shamsiYearStr1 = moment(date1).format('jYYYY/jM/jD').split("/")[0];
				var shamsiYearStr2 = moment(date2).format('jYYYY/jM/jD').split("/")[0];
				if(shamsiYearStr1 == shamsiYearStr2){
					return 0;
				}else{
					return (parseInt(shamsiYearStr1) - parseInt(shamsiYearStr2));
				}
		...
		link : function(scope, element, attrs, ngModel) {
			...
			function parseDate(viewValue) {
				if (!viewValue) {
					...
					// var date = uibDateParser.parse(viewValue, dateFormat) || new
					// Date(viewValue);
		...
		[
				...
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/popup.html", "<ul style=\"direction:rtl;\"  class=\"dropdown-menu datepicker-popup\" ng-style=\"{display: (isOpen && 'block') || 'none', top: position.top+'px', left: position.left+'px'}\" ng-keydown=\"keydown($event)\">\n" + "	<li ng-transclude  style=\"padding: 5px 9px;\"></li>\n"
							...
							+ "		<button type=\"button\" class=\"btn btn-sm btn-warning pull-left\" ng-click=\"close()\">{{ getText('close') }}</button>\n" + "	</li>\n" + "</ul>\n" + "");
				} ]);
